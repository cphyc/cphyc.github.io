#import "@preview/nth:0.2.0": nth
#let publist = (hl_author) => [

    I have submitted #highlight[13] articles as lead or co-lead author (#highlight[11] already published in MNRAS and A&A).
    I also contributed to #highlight[16] other articles.
    My papers have been cited #highlight[655] times (_h_-index of 14 as
    of #nth({26}) January 2025), #link("https://ui.adsabs.harvard.edu/search/filter_database_fq_database=AND&filter_database_fq_database=database%3A%22astronomy%22&fq=%7B!type%3Daqp%20v%3D%24fq_database%7D&fq_database=(database%3A%22astronomy%22)&p_=0&q=((author%3A%22Cadiou%2C%20C%22))&sort=date%20desc%2C%20bibcode%20desc")[source: NASA/ADS.]


    == Submitted articles

    +  “*RAMSES-yOMP: Performance Optimizations for the Astrophysical Hydrodynamic Simulation Code RAMSES*”,
      Han, Dubois, Lee, Kim, #hl_author[Cadiou]~\& Yi, _submitted to Monthly Notices of the Royal Astronomical Society_, (2024).    
    +  “*The Impact of Star Formation and Feedback Recipes on the Stellar Mass and Interstellar Medium of High-Redshift Galaxies*”,
      Katz, Rey, #hl_author[Cadiou], Kimm~\& Agertz, _submitted to Monthly Notices of the Royal Astronomical Society_, (2024).    
    +  “*The causal effect of cosmic filaments on dark matter halos*”,
      Storck, #hl_author[Cadiou], Agertz~\& Galárraga-Espinosa, _submitted to Monthly Notices of the Royal Astronomical Society_, (2024).    
    +  “*EDGE-INFERNO: Simulating every observable star in faint dwarf galaxies and their consequences for resolved-star photometric surveys*”,
      Andersson, Rey, Pontzen, #hl_author[Cadiou], Agertz, Read~\& Martin, _submitted to Monthly Notices of the Royal Astronomical Society_, (2024).    
    +  “*How complex are galaxies? A non-parametric estimation of the intrinsic dimensionality of wide-band photometric data*”,
      #hl_author[Cadiou], Laigle~\& Agertz, _submitted to Monthly Notices of the Royal Astronomical Society_, (2024).    

    == Published articles

    +  “*RAMSES-yOMP: Performance Optimizations for the Astrophysical Hydrodynamic Simulation Code RAMSES*”,
      Han, Dubois, Lee, Kim, #hl_author[Cadiou]~\& Yi, _in The Astrophysical Journal, 978, 1, 96-106_, (2025).    
    +  “*EDGE-INFERNO: Simulating Every Observable Star in Faint Dwarf Galaxies and Their Consequences for Resolved-star Photometric Surveys*”,
      Andersson, Rey, Pontzen, #hl_author[Cadiou], Agertz, Read~\& Martin, _in The Astrophysical Journal, 978, 2, 129-139_, (2025).    
    +  “*Running with the bulls: The frequency of star-disc encounters in the Taurus star-forming region*”,
      Winter, Benisty, Shuai, Dûchene, Cuello, Anania, #hl_author[Cadiou]~\& Joncour, _in Astronomy and Astrophysics, 691, A43_, (2024).    
    +  “*The AGORA High-resolution Galaxy Simulations Comparison Project. IV. Halo and Galaxy Mass Assembly in a Cosmological Zoom-in Simulation at z ≤ 2*”,
      Roca-Fàbrega, Kim, Primack, Jung, Genina, Hausammann, Kim, Lupi, Nagamine, Powell, Revaz, Shimizu, Strawn, Velázquez, Abel, Ceverino, Dong, Quinn, Shin, Segovia-Otero, Agertz, Barrow, #hl_author[Cadiou], Dekel, Hummels, Oh, Teyssier~\& AGORA Collaboration, _in The Astrophysical Journal, 968, 2, 125-154_, (2024).    
    +  “*Probing cosmology via the clustering of critical points*”,
      Shim, Pichon, Pogosyan, Appleby, #hl_author[Cadiou], Kim, Kraljic~\& Park, _in Monthly Notices of the Royal Astronomical Society, 528, 2, 1604-1615_, (2024).    
    +  “*Hot gas accretion fuels star formation faster than cold accretion in high-redshift galaxies*”,
      Kocjan, #hl_author[Cadiou], Agertz~\& Pontzen, _in Monthly Notices of the Royal Astronomical Society, 534, 1, 918-930_, (2024).    
    +  “*Estimating major merger rates and spin parameters ab initio via the clustering of critical events*”,
      #hl_author[Cadiou], Pichon-Pharabod, Pichon~\& Pogosyan, _in Monthly Notices of the Royal Astronomical Society, 531, 1, 1385-1398_, (2024).    
    +  “*Evolution of cosmic filaments in the MTNG simulation*”,
      Galárraga-Espinosa, #hl_author[Cadiou], Gouin, White, Springel, Pakmor, Hadzhiyska, Bose, Ferlito, Hernquist, Kannan, Barrera, Maria Delgado~\& Hernández-Aguayo, _in Astronomy and Astrophysics, 684, A63_, (2024).    
    +  “*Hot gas accretion fuels star formation faster than cold accretion in high redshift galaxies*”,
      Kocjan, #hl_author[Cadiou], Agertz~\& Pontzen, _in American Astronomical Society Meeting Abstracts, 243, 306.02_, (2024).    
    +  “*Stellar angular momentum can be controlled from cosmological initial conditions*”,
      #hl_author[Cadiou], Pontzen~\& Peiris, _in Monthly Notices of the Royal Astronomical Society, 517, 3, 3459-3469_, (2022).    
    +  “*Forecasts for WEAVE-QSO: 3D clustering and connectivity of critical points with Lyman-α tomography*”,
      Kraljic, Laigle, Pichon, Peirani, Codis, Shim, #hl_author[Cadiou], Pogosyan, Arnouts, Pieri, Iršič, Morrison, Oñorbe, Pérez-Ràfols~\& Dalton, _in Monthly Notices of the Royal Astronomical Society, 514, 1, 1359-1386_, (2022).    
    +  “*Gravitational torques dominate the dynamics of accreted gas at z > 2*”,
      #hl_author[Cadiou], Dubois~\& Pichon, _in Monthly Notices of the Royal Astronomical Society, 514, 4, 5429-5443_, (2022).    
    +  “*The causal effect of environment on halo mass and concentration*”,
      #hl_author[Cadiou], Pontzen, Peiris~\& Lucie-Smith, _in Monthly Notices of the Royal Astronomical Society, 508, 1, 1189-1195_, (2021).    
    +  “*Angular momentum evolution can be predicted from cosmological initial conditions*”,
      #hl_author[Cadiou], Pontzen~\& Peiris, _in Monthly Notices of the Royal Astronomical Society, 502, 4, 5480-5487_, (2021).    
    +  “*The clustering of critical points in the evolving cosmic web*”,
      Shim, Codis, Pichon, Pogosyan~\& #hl_author[Cadiou], _in Monthly Notices of the Royal Astronomical Society, 502, 3, 3885-3911_, (2021).    
    +  “*EDGE: a new approach to suppressing numerical diffusion in adaptive mesh simulations of galaxy formation*”,
      Pontzen, Rey, #hl_author[Cadiou], Agertz, Teyssier, Read~\& Orkney, _in Monthly Notices of the Royal Astronomical Society, 501, 2, 1755-1766_, (2021).    
    +  “*Tracing the simulated high-redshift circumgalactic medium with Lyman α emission*”,
      Mitchell, Blaizot, #hl_author[Cadiou], Dubois, Garel~\& Rosdahl, _in Monthly Notices of the Royal Astronomical Society, 501, 4, 5757-5776_, (2021).    
    +  “*The OBELISK simulation: Galaxies contribute more than AGN to H I reionization of protoclusters*”,
      Trebitsch, Dubois, Volonteri, Pfister, #hl_author[Cadiou], Katz, Rosdahl, Kimm, Pichon, Beckmann, Devriendt~\& Slyz, _in Astronomy and Astrophysics, 653, A154_, (2021).    
    +  “*When do cosmic peaks, filaments, or walls merge? A theory of critical events in a multiscale landscape*”,
      #hl_author[Cadiou], Pichon, Codis, Musso, Pogosyan, Dubois, Cardoso~\& Prunet, _in Monthly Notices of the Royal Astronomical Society, 496, 4, 4787-4822_, (2020).    
    +  “*Dense gas formation and destruction in a simulated Perseus-like galaxy cluster with spin-driven black hole feedback*”,
      Beckmann, Dubois, Guillard, Salome, Olivares, Polles, #hl_author[Cadiou], Combes, Hamer, Lehnert~\& Pineau des Forets, _in Astronomy and Astrophysics, 631, A60_, (2019).    
    +  “*Accurate tracer particles of baryon dynamics in the adaptive mesh refinement code Ramses*”,
      #hl_author[Cadiou], Dubois~\& Pichon, _in Astronomy and Astrophysics, 621, A96_, (2019).    
    +  “*Galaxies flowing in the oriented saddle frame of the cosmic web*”,
      Kraljic, Pichon, Dubois, Codis, #hl_author[Cadiou], Devriendt, Musso, Welker, Arnouts, Hwang, Laigle, Peirani, Slyz, Treyer~\& Vibert, _in Monthly Notices of the Royal Astronomical Society, 483, 3, 3227-3255_, (2019).    
    +  “*Galaxy evolution in the metric of the cosmic web*”,
      Kraljic, Arnouts, Pichon, Laigle, de la Torre, Vibert, #hl_author[Cadiou], Dubois, Treyer, Schimd, Codis, de Lapparent, Devriendt, Hwang, Le Borgne, Malavasi, Milliard, Musso, Pogosyan, Alpaslan, Bland-Hawthorn~\& Wright, _in Monthly Notices of the Royal Astronomical Society, 474, 1, 547-572_, (2018).    
    +  “*How does the cosmic web impact assembly bias?*”,
      Musso, #hl_author[Cadiou], Pichon, Codis, Kraljic~\& Dubois, _in Monthly Notices of the Royal Astronomical Society, 476, 4, 4877-4907_, (2018).    
]
